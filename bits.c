...
int byteNot(int x, int n) {
	int need_byte, mask, con, result;
	mask = 0xff << (n << 3);
	need_byte = mask & x;
	con = mask | x;
	result = ~need_byte & con;
	return result;
}
/* 
 ...
 */
int mult3div2(int x) {
	int temp, h_b, l_b, a_or_s, result;
	temp = x << 1;
	temp = temp + x;
	h_b = temp & (1 << 31);
	l_b = temp & 1;
	a_or_s = (~l_b + 1) & (((!!h_b) << 1) + ~0);
	result = (temp + a_or_s) >> 1;
	return result;
}
/* 
...
int subOK(int x, int y) {
	int x_sign, y_sign, sub, sub_sign, result;
	x_sign = (x >> 31) & 1;
	y_sign = (y >> 31) & 1;
	sub = x + ~y + 1;
	sub_sign = (sub >> 31) & 1;
	result = (x_sign ^ y_sign) & (sub_sign ^ x_sign);
	return !result;
}
/* 
